...
class Connection
{
     ...
     *
     * @param \Maleficarum\Api\Worker\Command\AbstractCommand $cmd
     *
     * @return \Maleficarum\Api\Rabbitmq\Connection
     */
    public function addCommand(\Maleficarum\Api\Worker\Command\AbstractCommand $cmd)
    {
    ...
    public function addCommands(array $cmds)
    {
        ...
        // validate commands
        if (count($cmds) < 1) {
            throw new \InvalidArgumentException('Expected a nonempty array of commands. \Maleficarum\Api\Rabbitmq\Connection::addCommands()');
        }

        foreach ($cmds as $cmd) {
            if (!$cmd instanceof \Maleficarum\Api\Worker\Command\AbstractCommand) {
                throw new \InvalidArgumentException('Not a valid command. \Maleficarum\Api\Rabbitmq\Connection::addCommands()');
            }
        }
