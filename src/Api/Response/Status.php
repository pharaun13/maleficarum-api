<?php
/**
 * This class is designed to provide a list of valid HTTP status codes and mapping of such codes
 * to RFC approved string messages.
 */
...
class Status
{
    ...
    {
        if (!is_int($code)) throw new \InvalidArgumentException('Incorrect HTTP code - integer expected. \Maleficarum\Api\Response\Http\Status::getMessageForStatus()');
        if (!array_key_exists($code, self::$recommendedReasonPhrases)) throw new \InvalidArgumentException('Unsupported HTTP code. \Maleficarum\Api\Response\Http\Status::getMessageForStatus()');
